...
static readonly profiles = {
  fast: {
    label: 'fast',
    description: 'Favor lower latency over strict verification extras'
  },
  balanced: {
    label: 'balanced',
    description: 'Balanced tradeoff between speed and safety'
  },
  paranoid: {
    label: 'paranoid',
    description: 'Enable all additional checks and validations'
  }
} as const;

static async benchmarkQueryProof(query: string): Promise<{
  totalMs: number;
  hashMs: number;
  proveMs: number;
  verifyMs: number;
  ...
  const start = now();
  const hashStart = now();
  const queryHash = await this.poseidonHash([query]);
  const hashMs = now() - hashStart;

  const proveStart = now();
  const { proof, publicSignals } = await this.createAndVerifyQueryProof({
    query
  });
...
static async proveInWorker(query: string): Promise<{
  proof: any;
  publicSignals: any;
  queryHash: string;
}> {
  return this.createAndVerifyQueryProof({ query });
}
}
